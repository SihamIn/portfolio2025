(function ($) {
    ...
    // Notif visite (Vercel API) - 1 fois par session (ou toutes X heures)
    (function notifyVisitOnce() {
        try {
            var cfg = (window.CONTACT_CONFIG && window.CONTACT_CONFIG.api) ? window.CONTACT_CONFIG.api : {};
            var path = cfg.visitPath || '/api/visit';
            var hours = Number(cfg.visitOncePerHours || 12);
            var key = 'visit_notified_at';
            var last = Number(localStorage.getItem(key) || '0');
            var now = Date.now();
            if (last && (now - last) < hours * 60 * 60 * 1000) return;

            var payload = JSON.stringify({ page: window.location.href });
